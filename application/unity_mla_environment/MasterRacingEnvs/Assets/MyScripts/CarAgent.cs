...
// ------------------------------ Methods. ---------------------------------- //
    ...
    private void _computeStepRewards(float throttle, float steeringAngle) {
        ...
        //     AddReward(OUT_OF_ROAD_PENALTY * 0.2f * (5 - roadIntersectRaysCount));
        // } else {
        //     // Add reward based on how close to the center of road the car is.
        //     AddReward(ROAD_CENTER_DISTANCE_REWARD * 0.1f * roadIntersectRaysCount);
        // }
...
// --------------------- Reward and penalty constants. ---------------------- //
    ...
    private const float THROTTLE_REWARD = 0.5f;
    // Discourage to do redundant steering wheel movements.
    private const float STEERING_ANGLE_PENALTY = -0.5f;
    // Discourage to pull off the road.
    private const float OUT_OF_ROAD_PENALTY = -1.0f;
    // Encourage to keep car on the center of the road.
    private const float ROAD_CENTER_DISTANCE_REWARD = 0.5f;
    // Encourage to keep the correct driving direction.
    private const float CAR_DIRECTION_REWARD = 0.5f;
